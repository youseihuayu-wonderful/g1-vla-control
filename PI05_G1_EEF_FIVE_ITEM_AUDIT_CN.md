...
- 四元数在送入 IK 前由消费者归一化；
- `g1-client` 已公开 full-remaining-horizon execution、prefetch、time-alignment 和 joint-space blend 参考；项目负责人已确认正式 cadence 为代码默认的 15 Hz，30 Hz help text 视为过期文档。
...
| 5. normalization/post-processing/replanning | checkpoint quantile norm；IK 前归一化；15 Hz full-horizon + prefetch/time-alignment/blend reference | 已确认 | 项目 sequential IK、collision、latency 和 watchdog Gate 仍需补齐 |
...
2. `control_hz=15` 已确认；formal manifest 仍必须显式写入，禁止依赖 CLI help/default 漂移；
...
5. 以已确认的 15 Hz manifest 完成 multi-chunk IK/swept-path，再运行 Adaptive-OFF MuJoCo 基线；
